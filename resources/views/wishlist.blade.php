...
@section('content')
            ...
            @if ($items->count() > 0)
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <h2 class="td-color">
                                                ...
                                                @endif
                                            <a href="javascript:void(0)" class="icon"
                                                onclick="removeWishlistItem('{{ $item->rowId }}')">
                                                <i class="fas fa-times"></i>
                                            </a>
                                        </td>
                                    </tr>
                ...
                <div class="row">
                    <div class="col-12 d-flex justify-content-between">
                        <a href="javascript:void(0)" class="btn btn-solid" onclick="clearWishlist()">Clear All Wishlist</a>
                        <a href="{{ route('shop.index') }}" class="btn btn-solid">Continue Shopping</a>
                    </div>
                </div>
            @else
                <div class="row">
    ...
    <!-- Cart Section End -->
    <form action="{{ route('wishlist.remove') }}" method="POST" id="removeWishlist">
        @csrf
        @method('DELETE')
    ...
    <form action="{{ route('wishlist.move.to.cart') }}" method="POST" id="moveToCartForm">
        @csrf
...
@push('scripts')
    <script>
        function removeWishlistItem(rowId) {
            $("#rowId").val(rowId);
            $("#removeWishlist").submit();
        }
